...
function Fanta() {
  ...
  const setFrontCamera = () => {
    if (orbitRef.current) {
      resetPosition();
      orbitRef.current.object.position.set(-20, 0, 50); // Üstten bakış için pozisyon
  ...
  const setBackView = () => {
    if (orbitRef.current) {
      resetPosition();
      orbitRef.current.object.position.set(-20, 0, -50); // Arkadan bakış için pozisyon
      orbitRef.current.object.lookAt(0, 0, 0); // Modelin merkezine bak
    }
  };
  ...
  const setLeftView = () => {
    if (orbitRef.current) {
      resetPosition();
      orbitRef.current.object.position.set(-50, 0, -20); // Soldan bakış için pozisyon
      orbitRef.current.object.lookAt(0, 0, 0); // Modelin merkezine bak
    }
  };
  ...
  const setRightView = () => {
    if (orbitRef.current) {
      resetPosition();
      orbitRef.current.object.position.set(50, 0, -20); // Sağdan bakış için pozisyon
      orbitRef.current.object.lookAt(0, 0, 0); // Modelin merkezine bak
    }
  };
  ...
  const setTopView = () => {
    if (orbitRef.current) {
      ...
      orbitRef.current.object.position.set(0, 50, 0); // Üstten bakış için pozisyon
      orbitRef.current.object.lookAt(0, 0, 0); // Modelin merkezine bak
    }
  };
  ...
  const setBottomView = () => {
    if (orbitRef.current) {
      resetPosition();
      orbitRef.current.object.position.set(0, -50, 0); // Aşağıdan bakış için pozisyon
      orbitRef.current.object.lookAt(0, 0, 0); // Modelin merkezine bak
    }
  };
  ...
  function FantaBottleModel() {
    const gltf = useGLTF("/~db596/assets/fantabottle.gltf", true); // Modelin yolu
  ...
  return (
    <div className="fanta">
      ...
      <Canvas className="fanta-bottle-canvas" camera={{ position: [-20, 0, 50], fov: 40 }}>
